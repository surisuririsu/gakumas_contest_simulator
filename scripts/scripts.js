...
document.addEventListener(
  ...
  () => {
    // カードセレクトを挿入
    const element_main_card_box = document.getElementById(
      "main-character-cards-box"
    );
    const element_sub_card_box = document.getElementById(
      "sub-character-card-box"
    );

    const element_main_cards = DOM_set_characterCards(
      element_main_card_box,
      6,
      "main"
    );
    const element_sub_cards = DOM_set_characterCards(
      element_sub_card_box,
      6,
      "sub"
    );

    const element_pItem_box = document.getElementById(
      "main-character-pItem-box"
    );
    const element_pItems = DOM_set_characterCards(
      element_pItem_box,
      4,
      "pItem"
    );
    ...
    Array.from(document.querySelectorAll(".character-cards select")).forEach(
      (element) => {
    ...
    function DOM_set_select_contest_pItem(id) {
      ...
      const pItemId =
        ContestData.getById(contestId).stages[stageId].stagePItemIds[0];
    ...
    function DOM_set_select_normal_pItem() {
      ...
      const list = PItemData.getAll().filter(
        (item) =>
          String(item.id)[0] == "3" && // サポートのPアイテム
          item.id % 10 == 0 && // 未強化
          item.effects
      );
      element_selects.forEach((element) =>
        DOM_set_select_options(element, list, true)
      );
    }
    ...
    function DOM_set_uniqueSkillCard(type, id) {
      const element_select =
        type == "main" ? element_main_cards[0] : element_sub_cards[0];
    ...
    function DOM_set_otherSkillCards(plan) {
      const elements = element_main_cards
        .slice(1)
        .concat(element_sub_cards.slice(1));
      const skillCards = SkillCardData.getAll().filter(
        (item) =>
    ...
    const element_main_character_select =
      document.getElementById("main-character");
    const element_sub_character_select =
      document.getElementById("sub-character");

    element_main_character_select.addEventListener(
      "change",
      (e) => {
        ...
        const dPIdols = PIdolData.getByCharacterId(pIdol.character_id).filter(
          (idol) => idol.plan == pIdol.plan
        );
    ...
    element_sub_character_select.addEventListener(
      "change",
      (e) => {
        const id = Number(e.target.value);
        const pIdol = PIdolData.getById(id);
    ...
    const element_contest_stage_select = document.getElementById(
      "contest-stage-select"
    );
    DOM_set_contest(element_contest_select);
    element_contest_select.options[
      element_contest_select.options.length - 1
    ].selected = true;
    ...
    element_contest_select.addEventListener("change", (e) => {
      ...
      contestDetail.stages.forEach((item, i) => {
        const option = document.createElement("option");
        const translate_plan = {
          free: "フリー",
          sense: "センス",
          logic: "ロジック",
        };
        option.innerHTML = `${item.name}(${item.turn}T/${
          translate_plan[item.plan]
        })`;
        option.value = `${id}:${i}`;
        fragment.appendChild(option);
      });
    ...
    element_contest_stage_select.addEventListener("change", (e) => {
      ...
      if (current_stage_plan != plan) {
        ...
        // メインキャラクター設定
        DOM_set_character(
          element_main_character_select,
          PIdolData.getAll().filter((idol) =>
            plan == "free" ? true : plan == idol.plan
          )
        );
        element_sub_character_select.disabled = true;
    ...
    function loadOptionsFromSearchParams() {
      ...
      const pItems = urlParams.get("items");
      ...
      if (pItems) {
        ...
        element_pItems
          .slice(2)
          .forEach((elem, idx) => (elem.value = itemIds[idx]));
      }
      if (cards) {
    ...
    element_run_button.addEventListener(
      ...
      () => {
        ...
        // Pアイテム
        const pItemIds_tmp = element_pItems
          .map((element) => Number(element.value))
          .filter((id) => id != -1);
        if (
          element_pItems[1].parentNode.getElementsByClassName("checkbox")[0]
            .checked
        ) {
          pItemIds_tmp[1] += 1;
        }
        ...
        // カード
        const main_cards_id = element_main_cards.map(
          (element) =>
            Number(element.value) +
            (element.parentNode.getElementsByClassName("checkbox")[0].checked
              ? 1
              : 0)
        );
        const sub_cards_id = element_sub_cards.map(
          (element) =>
            Number(element.value) +
            (element.parentNode.getElementsByClassName("checkbox")[0].checked
              ? 1
        ...
        const skillCardIds = main_cards_id
          .concat(sub_cards_id)
          .filter((id) => id && id != "-1");
        // 重複を削除する機構がないため、仮削除
        if (element_main_cards[0].value == element_sub_cards[0].value) {
          let delete_number = skillCardIds.lastIndexOf(
            Number(element_main_cards[0].value)
          );
          if (delete_number == -1) {
            delete_number = skillCardIds.lastIndexOf(
              Number(element_main_cards[0].value) + 1
            );
          }
        ...
        // コンテスト
        const [contestId, stageId] =
          element_contest_stage_select.value.split(":");
        ...
        document.getElementById("contest-log").innerHTML =
          minLog.text.replaceAll("\n", "<br>");
        ...
        document.getElementById("result-score-median").textContent =
          scoreList.sort()[Math.floor(scoreList.length / 2)];
